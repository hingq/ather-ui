...
// 文件解析回调
const fileCallback = props.fileHandleCallback

// 上传过程回调
const upload = props.uploadCallback

// 上传进度条回调
const progress = props.progressLoop

const successCallback = props.success
const failCallback = props.fail
onMounted(() => {
  const modal = new UploadModal('#upload')

  modal.fileCallback(fileCallback)
  modal.upload(upload)
  modal.progressLoop(progress)
  modal.success(successCallback)
  modal.fail(failCallback)
})
</script>
